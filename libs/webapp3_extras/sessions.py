...
Lightweight but flexible session support for webapp3.
"""
import re
...
from webapp3_extras import securecookie
...
default_config = {
    ...
    'backends': {
        'securecookie': 'webapp3_extras.sessions.SecureCookieSessionFactory',
        'datastore':    'webapp3_extras.appengine.sessions_ndb.'
                        'DatastoreSessionFactory',
        'memcache':     'webapp3_extras.appengine.sessions_memcache.'
...
class SessionStore(object):
    ...
    def get_backend(self, name):
        ...
        if isinstance(backend, six.string_types):
            backend = backends[name] = webapp3.import_string(backend)
...
_registry_key = 'webapp3_extras.sessions.SessionStore'
...
def get_store(factory=SessionStore, key=_registry_key, request=None):
    ...
    """
    request = request or webapp3.get_request()
